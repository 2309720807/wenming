...
- **地图与探索界面**（`scenes/ui/explore/explore_map.tscn`，默认进入）：
  ...
  - **地图扩大/查看**：右上角"地图"按钮**弹出扩大地图购买面板**（当前地图尺寸/金币/三档扩大方案，确认后扣费，尺寸随存档持久化）；地图渲染限制在地图显示区域内（裁剪不溢出覆盖菜单/信息面板等 UI）；未选中建筑时**左键拖动平移地图**查看；滚轮以光标为锚点缩放（0.4x-3.0x，**切换界面保持并跨启动记忆**）；选中建筑后左键拖动为连续建造
