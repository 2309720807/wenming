...
- **地图与探索界面**（`scenes/ui/explore/explore_map.tscn`，默认进入）：
  - **3D 化地图（参考《模拟城市：我是市长》）**：中央网格区为 3D 子视口渲染（SubViewport + Node3D），以真实建筑为模板的程序化低多边形建筑（`BuildingMeshes` 模块：住宅=多层住宅楼、办公楼=玻璃幕墙写字楼、学校=校园教学楼+钟楼+操场、医院=医院大楼+停机坪、金融中心=分段收分摩天大楼、路灯=灯柱+发光灯罩（夜间点亮），数据驱动、升级长高），树/岩石/湖泊 3D 化；**轨道摄像机：滚轮缩放、左键拖动三维旋转、中键/右键拖动平移；射线拾取建造/清障/选中**；等级徽章随缩放比例变化；**建筑墙面纹面**（程序化砖纹/瓷砖/玻璃幕纹理）；**昼夜光影**（太阳/月亮东升西落，天空渐变）
  - 部落冲突式网格建设：左侧可滚动建筑菜单栏 + 中央 25×14 网格建设区（1000×560px，居中填满）
  - 6 种建筑（住宅/办公楼/学校/医院/金融中心/路灯）+ 3 种装饰（花园/喷泉/雕像），数据配置于 `data/buildings.json`
  ...
  - **批量升级**：右上角"⬆ 升级"框选区域（3D 青色高亮选区），确认后已完工建筑批量升级（进入施工，施工时间 = 建造时间×0.6×当前等级，与单次升级一致，费用随等级递增）
...
- **军事与防御系统**（`scenes/ui/military/military_view.tscn` + `scripts/ui/military_view.gd` + `scripts/data/military_system.gd` Autoload）：
  ...
  - 军事页消耗金币+科技点制造设施，入库存（**支持批量制造 ×1/×5/×10**）；**防御型**（炮塔/城墙等）建基地守城、**进攻型**（剑士/弓箭手/骑兵/攻城锤/投石车）副本进攻；区分**远程/近战**攻击方式
  - 防御页"⬆ 批量升级"框选区域内设施升级（每级攻击/生命+25%，消耗金币+科技）
