...
- **地图与探索界面**（`scenes/ui/explore/explore_map.tscn`，默认进入）：
  - **3D 化地图（参考《模拟城市：我是市长》）**：中央网格区为 3D 子视口渲染（SubViewport + Node3D），程序化低多边形仿真建筑（住宅斜屋顶/办公楼玻璃幕墙/学校旗杆/医院红十字/金融中心尖顶，数据驱动），树/岩石/湖泊 3D 化；**轨道摄像机：滚轮缩放、左键拖动三维旋转、中键/右键拖动平移；射线拾取建造/清障/选中**；等级徽章随缩放比例变化
