...
- **地图与探索界面**（`scenes/ui/explore/explore_map.tscn`，默认进入）：
  - **3D 化地图（参考《模拟城市：我是市长》）**：中央网格区为 3D 子视口渲染（SubViewport + Node3D），以真实建筑为模板的程序化低多边形建筑（`BuildingMeshes` 模块：住宅=多层住宅楼、办公楼=玻璃幕墙写字楼、学校=校园教学楼+钟楼+操场、医院=医院大楼+停机坪、金融中心=分段收分摩天大楼，数据驱动、升级长高），树/岩石/湖泊 3D 化；**轨道摄像机：滚轮缩放、左键拖动三维旋转、中键/右键拖动平移；射线拾取建造/清障/选中**；等级徽章随缩放比例变化；**建筑墙面纹面**（程序化砖纹/瓷砖/玻璃幕纹理）；**昼夜光影**（太阳/月亮东升西落，天空渐变）
  ...
  - **批量升级**：右上角"⬆ 升级"框选区域（3D 青色高亮选区），确认后已完工建筑批量升级（立即完成，费用随等级递增）
